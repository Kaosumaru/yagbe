...
public:
    ...
    base_sound(bit enabled, bit left_enabled, bit right_enabled)
        : _enabled(enabled)
        , _left_enabled(left_enabled)
        , _right_enabled(right_enabled)
    {
    }
    ...
    {
        if (!_enabled)
            return; //TODO probably should pause everything in sound circuit
        sample *= _maxVolume;
        if (_left_enabled)
            channels[1] = sample;
        if (_right_enabled)
...
protected:
    ...
    public:
        Counter(Control& control, bit enabled)
            : _control(control)
            , _enabled(enabled)
        {
        }
    ...
    public:
        ...
        float get_volume(double delta)
        {
            ...
            while (_envAcc > envLength)
            {
                ...
                if (_envelopeVolume > _maxEnvelopeVolume)
                    _envelopeVolume = _maxEnvelopeVolume;
